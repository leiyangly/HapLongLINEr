...
Run `haplongliner`, `haplongliner rm` (RM module), `haplongliner sv` (SV module) or
`haplongliner db` with no arguments to see the available options for each command.

### RM module: Based on RepeatMasker pre-masked genome assemblies
...
### SV module: Based on structural variation calls, no RepeatMasker pre-masking of whole assembly needed
...
- Reference genome FASTA (hs1, hg38, or custom; default hs1)
- Transposable element families to analyse (via `--te`, default `L1,L1PA3`)
- The built-in pangenome L1 reference was generated by the RM module using 94 haplotypes of HPRC release 1, ≥5kb cutoff, plus the L1s that were present in the reference genome (hs1 and hg38) but not in any of the 94 HPRC haploids
...
This module retrieves the sequences of L1 insertions present in the HPRC
